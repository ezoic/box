...
var (
	...
	login = flag.String("login", "", "login for the new user")
	name  = flag.String("name", "", "name for the new user")
)
...
func main() {
	...
	resp, user, err := c.UserService().CreateUser(&box.User{
		Login: *login,
		Name:  *name,
	})
